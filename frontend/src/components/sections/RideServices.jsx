                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
                    <div className="max-w-2xl">
                        ...
                        <h2 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tighter mb-6">
                            Ride & Delivery <span className="text-primary italic">Reimagined.</span>
                        </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {rides.map((ride, idx) => (
                        <motion.div 
                            ...
                            className="glass-card p-10 rounded-[3rem] bg-white dark:bg-dark-card border border-slate-100 dark:border-slate-800 shadow-2xl group hover:-translate-y-2 transition-all relative overflow-hidden"
                        >
                            <div className={`w-16 h-16 ${ride.color} rounded-2xl flex items-center justify-center mb-8 shadow-2xl shadow-primary/20 relative z-10`}>
                                <ride.icon size={28} className="text-white" />
                            </div>
                            
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-4 relative z-10">{ride.title}</h3>
                            <p className="text-slate-500 dark:text-slate-400 font-medium mb-8 text-sm leading-relaxed relative z-10">
                                {ride.desc}
                            </p>
                            ...
                            <div className="flex items-center justify-between pt-6 border-t border-slate-50 dark:border-slate-800 relative z-10">
                                <div>
                                    ...
                                    <p className="text-xl font-black text-primary">{ride.price}</p>
                                </div>
                                <button className="w-12 h-12 rounded-full bg-slate-50 dark:bg-dark-bg flex items-center justify-center text-slate-400 group-hover:bg-primary group-hover:text-white transition-all">
                                    <ArrowRight size={20} />
                                </button>
                            </div>

                            {/* Background Number */}
                            <span className="absolute -bottom-10 -right-5 text-[150px] font-black text-slate-50/50 dark:text-white/5 select-none pointer-events-none">
                                0{idx + 1}
                            </span>
                        </motion.div>
                    ))}
